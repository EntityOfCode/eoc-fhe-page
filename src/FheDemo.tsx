...
const FheDemo: React.FC = () => {
    ...
    (async () => {
      try {
        ...
        module.onRuntimeInitialized = () => {
          logWithTimestamp(initTag, "Module initialized successfully.");
          setModule(module);
        ...
        if (module.calledRun) {
          ...
          logWithTimestamp(initTag, "Waiting for runtime initialization...");
        }
      } catch (error) {
        console.timeEnd("Module Initialization");
        logWithTimestamp("Module Initialization", `Error initializing the module: ${error}`);
      }
    })();
  }, []);
  ...
  return (
    <div className={styles.container}>
     ...
     <button onClick={generateSecretKey} disabled={loading} className={styles.button}>Generate Secret Key</button>
            {secretKey && <p>Secret Key: Generated</p>}
            <button onClick={generatePublicKey} disabled={loading || !secretKey} className={styles.button}>Generate Public Key</button>
            ...
            <div>
              ...
              />
              <button onClick={encryptInteger1} disabled={loading || !secretKey} className={styles.button}>Encrypt Value 1</button>
              ...
              <input
                ...
                className={styles.input}
              />
              <button onClick={encryptInteger2} disabled={loading || !secretKey} className={styles.button}>Encrypt Value 2</button>
            ...
            <div>
              ...
              <input
                ...
                className={styles.input}
              />
              <button onClick={encryptStringValue} disabled={loading || !secretKey} className={styles.button}>Encrypt String</button>
              {encryptedString && <p>Encrypted String: Generated</p>}
            </div>
            
            <div>
              <h2 className={styles.heading}>Operations on Encrypted Values</h2>
              <button onClick={addEncryptedValues} disabled={!encryptedValue1 || !encryptedValue2 || !publicKey} className={styles.button}>Add Encrypted Values</button>
              {encryptedSum && <p>Encrypted Sum: Generated</p>}
              <button onClick={subtractEncryptedValues} disabled={!encryptedValue1 || !encryptedValue2 || !publicKey} className={styles.button}>Subtract Encrypted Values</button>
              {encryptedDiff && <p>Encrypted Difference: Generated</p>}
            </div>
            
            <div>
              <h2 className={styles.heading}>Decrypt Results</h2>
              <button onClick={decryptSum} disabled={loading || !encryptedSum || !secretKey} className={styles.button}>Decrypt Sum</button>
              {decryptedSum !== null && <p>Decrypted Sum: {decryptedSum}</p>}
              <button onClick={decryptDiff} disabled={loading || !encryptedDiff || !secretKey} className={styles.button}>Decrypt Difference</button>
              {decryptedDiff !== null && <p>Decrypted Difference: {decryptedDiff}</p>}
              <button onClick={decryptStringValue} disabled={loading || !encryptedString || !secretKey} className={styles.button}>Decrypt String</button>
