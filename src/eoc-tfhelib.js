...
/** not-t@type {!BigUint64Array} */
        ...
        function findWasmBinary() {
            ...
            return new URL('eoc-tfhelib.wasm', import.meta.url).href
        }
        ...
        var integerReadValueFromPointer = (name, width, signed) => {
            ...
            switch (width) {
                ...
                case 2:
                    return signed
                        ? (pointer) => HEAP16[pointer / 2]
                        : (pointer) => HEAPU16[pointer / 2]
                case 4:
                    return signed
                        ? (pointer) => HEAP32[pointer / 4]
                        : (pointer) => HEAPU32[pointer / 4]
                case 8:
                    return signed
                        ? (pointer) => HEAP64[pointer / 8]
                        : (pointer) => HEAPU64[pointer / 8]
                default:
                    throw new TypeError(
        ...
        function readPointer(pointer) {
            return this['fromWireType'](Number(HEAPU64[pointer / 8]))
        }
            ...
            switch (width) {
                case 4:
                    return function (pointer) {
                        return this['fromWireType'](HEAPF32[pointer / 4])
                    }
                case 8:
                    return function (pointer) {
                        return this['fromWireType'](HEAPF64[pointer / 8])
                    }
                default:
                    throw new TypeError(
        ...
        var heap32VectorToArray = (count, firstElement) => {
            ...
            for (var i = 0; i < count; i++) {
                ...
                array.push(Number(HEAPU64[(firstElement + i * 8) / 8]))
            }
            return array
        }
        ...
        function __embind_register_memory_view(rawType, dataTypeIndex, name) {
            ...
            function decodeMemoryView(handle) {
                var size = Number(HEAPU64[handle / 8])
                var data = Number(HEAPU64[(handle + 8) / 8])
        ...
        var __embind_register_std_wstring = function (rawType, charSize, name) {
            ...
            if (charSize === 2) {
                ...
                readCharAt = (pointer) => HEAPU16[pointer / 2]
            } else if (charSize === 4) {
                decodeString = UTF32ToString
                encodeString = stringToUTF32
                lengthBytesUTF = lengthBytesUTF32
                readCharAt = (pointer) => HEAPU32[pointer / 4]
            }
            registerType(rawType, {
                name,
                fromWireType: (value) => {
                    // Code mostly taken from _embind_register_std_string fromWireType
                    var length = Number(HEAPU64[value / 8])
                    var str

                    var decodeStartPtr = value + 8
                    // Looping here to support possible embedded '0' bytes
                    for (var i = 0; i <= length; ++i) {
                        var currentBytePtr = value + 8 + i * charSize
                ...
                toWireType: (destructors, value) => {
                    ...
                    HEAPU64[ptr / 8] = BigInt(length / charSize)
            ...
            var ret = (() => {
                ...
                function getWeekBasedYear(date) {
                    ...
                    if (compareByDay(firstWeekStartThisYear, thisDate) <= 0) {
                        ...
                        if (
                            compareByDay(firstWeekStartNextYear, thisDate) <= 0
                        ) {
                ...
                var EXPANSION_RULES_2 = {
                    ...
                    '%V': (date) => {
                        ...
                        if (!val) {
                            ...
                            var dec31 =
                                (date.tm_wday + 7 - date.tm_yday - 1) % 7
                            if (
                                dec31 == 4 ||
                                (dec31 == 5 &&
                                    isLeapYear((date.tm_year % 400) - 1))
                            ) {
                                val++
                            }
                        } else if (val == 53) {
        ...
        function applySignatureConversions(wasmExports) {
            ...
            wasmExports['__cxa_is_pointer_type'] = makeWrapper__p(
                wasmExports['__cxa_is_pointer_type']
            )
            return wasmExports
        }
